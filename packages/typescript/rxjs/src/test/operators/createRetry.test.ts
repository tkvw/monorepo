...
import {tap,concatMap, merge,mergeMap,switchMap, retryWhen,iif, throwError, timer,mapTo, BehaviorSubject, mergeMapTo, Observable, of, finalize} from 'rxjs';
import createRetry from "../../operators/createRetry"


describe('createRetry',() => {
  it('test simple',marbles(m => {
    const source = m.cold('a--b--c|');
    const expected = m.cold('a--b--c|');
    m.expect(source).toBeObservable(expected);
  }));
  it('notifies messages',marbles(m => {
    const source = m.cold('a-#');
    const {retry,retryMessages} = createRetry({
      maxAttempts: of(2)
    });
    const expected = m.cold('a- 1000ms a- 4000ms a-#');
