...
export interface RxQueryOptions<TVariables, TData> extends WatchQueryOptions<TVariables, TData> {
  skip?: boolean;
}
            ...
            iif(
              ...
              new Observable<ApolloQueryResult<TData>>((observer) => {
                ...
                const fetchMoreSubscription = fetchMoreOptions
                  .pipe(switchMap((moreOptions) => from(watchQuery.fetchMore(moreOptions))))
                  .subscribe(observer);
